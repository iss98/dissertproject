...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const q = query(itemsRef, where("category", "==", "prior"));
    ...
    if (items.length === 0) {
      examDescription.textContent = "사전 검사 문항이 없습니다.";
      showMessage("items 컬렉션에 category가 prior인 문항이 없습니다.", "error");
      return;
    }

    examDescription.textContent = `총 ${items.length}개의 사전 검사 문항입니다.`;
